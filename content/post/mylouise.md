---
title: "MyLouiseProject API应用"
date: 2021-08-13T15:23:00+08:00
subtitle: ""
draft: false
comments: true
categories: 开发心得
...
>
> ```text
> .├── go-cqhttp├── config.yml (旧版: config.hjson)├── device.json├── filter	└── filters.json├── logs│   └── xx-xx-xx.log└── data ├── images │   └── xxxx.image └── db
> ```
>
> ```yaml
> # config.yml# 默认中间件锚点default-middlewares: &default# 事件过滤器文件目录# 编写默认上报信息的过滤器，满足条件的消息才会上报到API应用filter: 'filter/filters.json'
> ```
>
> ```json
> // filter.json// 规则含义 允许上报以半角!开头或者meta_event_type为心跳检测的消息{ ".or" :  [     {         "raw_message": {             ".regex": "^!"         }     },      {         "meta_event_type": "heartbeat"     } ]}
> ```
